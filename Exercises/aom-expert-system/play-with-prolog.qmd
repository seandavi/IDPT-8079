...
If you want to try with different symptoms, you can edit the facts at the top of the program and then click the "check_diagnosis_and_treatment" button to run the program again. Remember that putting a `%` mark at the beginning of the line turns the line into a comment, so it any line (or symptom) that starts with `%` will be ignored by the Prolog interpreter. 
